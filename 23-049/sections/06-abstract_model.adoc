...
This attempt at a Temporal Abstract Conceptual Model follows <<iso19111,ISO 19111>>, which is the ISO adoption of <<OGC_18-005r4>>.
...
ReferenceSystem o-- SpatialReferenceSystem : is a
ReferenceSystem o-- TemporalReferenceSystem : ia a
class OrdinalTemporalReferenceSystem (
       dimension = 1
       applicationLocationTimeOrDomain
    }
...
note for TemporalReferenceSystem "Note: Consists of one only of:\nTemporalCoordinateReferenceSystem,\nCalendar, or \nOrdinalTemporalReferenceSystem"
    TemporalReferenceSystem o-- OrdinalTemporalReferenceSystem: is a
    TemporalReferenceSystem o-- TemporalCoordinateReferenceSystem : is a
    TemporalReferenceSystem o-- Calendar : is a
...
[example]
A long ice core is retrieved from a stable ice-sheet. From long term meteorological observations, the rate of accumulation of ice is known, so linear length can be equated to time (assuming a stable climate too). This enable the dates of some previously unknown large scale volcanic eruptions to be identified and timed. Identifiable nuclear fallout from specific atmospheric atomic bomb tests increase the confidence in the timing accuracy.
...
[example]
Teams controlling remote vehicles on Mars use a solar calendar, with Martian years and martian days (called sols). Months are not used because there are two moons, with different, rather short, 'months'.
