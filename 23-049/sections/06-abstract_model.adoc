...
This paper only addresses the internationally agreed Gregorian calendar. <<astro_algo,Astronomical Algorithms>> by Jean Meeus, and <<calendrical,Calendrical Calculations>> by Nachum Dershowitz and Edward M. Reingold provide overwhelming detail for conversion to numerous other calendars that have developed around the world and over the millennia and to meet the various social needs of communities, whether agricultural, religious or other. The references are comprehensive but not exhaustive, as there are calendars that have been omitted.
