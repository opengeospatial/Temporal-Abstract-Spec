...
. Notation: An Ordinal Temporal Reference System 'consists of' an ordered set of <<events_section,Events>>. These events are identifiable temporal instances. 
...
An Algorithm specifies the logic used to construct a Timeline from its constituent <<timescale_section,Timescales>>. A Timeline does not have any attributes of its own. Nor does it make use of any other classes from this Temporal model.
