...
Calendars combine different timescales and their clocks and units of measure, and other events, to make a complex timeline against which events can be compared. Calculated algorithms are used to determine which instants or intervals on the compound timescale are identified and labeled.
...
Temporal constructs such as instants, durations or intervals, multi-instants (a set of instants), and multi-intervals are not included in this conceptual model. They are well described in <<temporal_knowledge,Maintaining Knowledge about Temporal Intervals by J. F. Allen>>  so do not need to be in this Abstract Conceptual Model.
